        ...
        <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex justify-content-between align-items-start flex-wrap gap-2">
                <div>
                    ...
                    <div class="fw-semibold">{{ venta.cliente?.usuario?.nombre_completo }}</div>
                    <div class="small"><i class="bi bi-geo-alt me-1"></i><strong>Dirección:</strong> {{ venta.cliente?.usuario?.direccion || 'No registrada' }}</div>
                    <div class="small"><i class="bi bi-telephone me-1"></i>{{ venta.cliente?.usuario?.telefono || '—' }}</div>
                    <div class="small text-muted mt-1">{{ new Date(venta.fecha).toLocaleString() }} · Vendedor: {{ venta.vendedor?.nombre_completo || '—' }} · Total {{ fmt(venta.monto_total) }}</div>
                </div>
