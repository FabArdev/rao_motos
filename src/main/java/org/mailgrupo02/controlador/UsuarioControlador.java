...
import org.mailgrupo02.modelo.dao.ClienteM;
import org.mailgrupo02.modelo.dao.UsuarioM;
import org.mailgrupo02.modelo.servicio.UsuarioService;
import org.mailgrupo02.modelo.entidad.UsuarioN;
import org.mailgrupo02.vista.PUsuarios;
...
public class UsuarioControlador {
    ...
    public static String handle(String cmd, List<String> params, String correoRemitente) {
        try {
            ...
            switch (cmd.toUpperCase()) {
                ...
                case "CREATEUSUARIO": {
                    if (params.size() < 6)
                        return PUsuarios.generarHtml(cmd, "Error: se requieren 6 par&aacute;metros [nombre,correo,password,rol,telefono,direccion].");
                ...
                case "UPDATEUSUARIO": {
                    if (params.size() < 8)
                        return PUsuarios.generarHtml(cmd, "Error: se requieren 8 par&aacute;metros [id,nombre,correo,password,rol,telefono,direccion,activo].");
                    int id = Integer.parseInt(params.get(0).trim());
                ...
                case "ACTUALIZARPERFIL": {
                    ...
                    org.mailgrupo02.modelo.entidad.UsuarioN antes = null;
                    try {
                        int selfId = service.buscarIdPorCorreo(correoRemitente);
                        if (selfId < 0) return PUsuarios.generarHtml(cmd, PedidoControlador.msgNoRegistrado(correoRemitente));
                        antes = service.leerUsuario(selfId);
                    } catch (Exception e) {
                        return PUsuarios.generarHtml(cmd, "Error: " + e.getMessage());
                    }
                    String msg = service.actualizarPerfil(correoRemitente,
                        params.get(0).trim(), params.get(1).trim(),
                        params.get(2).trim(), params.get(3).trim());
                    int newId = extraerId(msg);
                    if (newId > 0) {
                        org.mailgrupo02.modelo.entidad.UsuarioN despues = service.leerUsuario(newId);
                        return PUsuarios.generarHtml(cmd, diffUsuario(antes, despues));
                    }
                    return PUsuarios.generarHtml(cmd, msg);
                }
    ...
    private static String fichaUsuario(UsuarioN u, String tipo) {
        ...
        fila(sb, "Correo",            nvl(u.getCorreo()));
    ...
    private static String diffUsuario(UsuarioN a, UsuarioN d) {
        ...
        difFila(sb, "Correo",            nvl(a.getCorreo()),     nvl(d.getCorreo()));
    ...
    private static String diffCliente(UsuarioN usu, ClienteM a, ClienteM d) {
        ...
        sb.append("<p style=\"font-size:14px;color:#374151;margin:10px 16px;\">")
          ...
          .append(" &mdash; ").append(nvl(usu.getCorreo()))
