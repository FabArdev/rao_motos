...
public class PagoControlador {
        ...
        try {
            ...
            switch (cmd.toUpperCase()) {
                ...
                case "MISCREDITOS": {
                    ...
                    if (clienteId < 0) return PPagos.generarHtml(cmd, PedidoControlador.msgNoRegistrado(correoRemitente));
                ...
                case "ESTADOCUENTA": {
                    int clienteId = usuarioService.buscarIdPorCorreo(correoRemitente);
                    if (clienteId < 0) return PPagos.generarHtml(cmd, PedidoControlador.msgNoRegistrado(correoRemitente));
                    return PPagos.generarHtml(cmd, estadoCuenta(clienteId, correoRemitente));
                }
    ...
    private static String estadoCuenta(int clienteId, String correo) {
        StringBuilder sb = new StringBuilder();
        try {
            UsuarioM u = UsuarioM.leer(clienteId);
            String nombre = u != null && u.getNombre() != null ? u.getNombre() : correo;
            ...
            sb.append("<div style=\"padding:10px 16px;font-size:13px;color:#374151;\">")
              .append("<strong>Correo:</strong> ").append(correo).append("</div>");
            sb.append("</div>");
